...
private:
    ...
    // Each entry in batches_ is an Eigen::Matrix of shape:
    //   [batch_size_ x (number_of_rows_*number_of_columns_)]
    // except possibly for the last batch, which might have fewer rows if not divisible.
...
public:
    ...
    // Read MNIST images from file, store them in batch-sized chunk matrices
    void readImageData(const std::string &filepath);

    // Write a single image (by absolute index) to a file in the reference tensor format
    void writeImageToFile(const std::string &filepath, size_t index);

    // Return the matrix for the i-th batch
...
{
    // We interpret "index" as an absolute index across all images.
    // Which batch is that in, and which row inside that batch?
    size_t batch_no = index / batch_size_;
    size_t row_in_batch = index % batch_size_;

    if(batch_no >= batches_.size()) {
        std::cerr << "Index out of range: no such batch." << std::endl;
        return;
    }
    ...
    // Write rank=2
    output_file << 2 << "\n";
    // shape: number_of_rows_ x number_of_columns_
    output_file << number_of_rows_ << "\n";
    output_file << number_of_columns_ << "\n";

    // Dump the actual pixel data
    size_t image_size = number_of_rows_ * number_of_columns_;
